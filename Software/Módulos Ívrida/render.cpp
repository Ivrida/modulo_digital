/*
 * Archivo render para Bela, integrando 2 sensores ultrasónicos hcsr04 y TRILL hex y ring para patches de Pd
 * libpd.
 */
#include <Bela.h>
...
unsigned int gTriggerDigitalOutChannel = 0; //channel to be connected to the module's TRIGGER pin - check the pin diagram in the IDE
...
unsigned int gTriggerDigitalOutChannel2 = 2; //channel to be connected to the module's TRIGGER pin - check the pin diagram in the IDE
unsigned int gEchoDigitalInPin2 = 3; //channel to be connected to the modules's ECHO pin (digital pin 1) - check the pin diagram in the IDE
int gTriggerCount = 0;
int gPrintfCount = 0;
...
bool setup(BelaContext *context, void *userData)
{
    ...
    pinMode(context, 1, gEchoDigitalInPin, INPUT); // receiving from ECHO PIN
    ...
    if(context->analogInChannels != 8){
   	...
   	// Set the mode of digital pins
    pinMode(context, 0, gTriggerDigitalOutChannel2, OUTPUT);
    pinMode(context, 1, gEchoDigitalInPin2, INPUT); // receiving from ECHO PIN
...
{
			if(myHex.setup(1, Trill::HEX) != 0) {
				fprintf(stderr, "Unable to initialise touch sensor\n");
				return false;
			}
			if(myRing.setup(1, Trill::RING) != 0) {
				fprintf(stderr, "Unable to initialise touch sensor\n");
				return false;
			}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
	for(unsigned int n = 0; n<context->digitalFrames; n++){
        ...
        if(gTriggerCount == gTriggerInterval){
           ...
           digitalWriteOnce(context, n, gTriggerDigitalOutChannel, LOW);
    ...
    for(unsigned int n = 0; n<context->digitalFrames; n++){
        ...
        if(gTriggerCount2 == gTriggerInterval){
            gTriggerCount2 = 0;
            digitalWriteOnce(context, n, gTriggerDigitalOutChannel2, HIGH);
           //analogWriteOnce(context, n / 2, gTriggerAnalogOutChannel, HIGH); //write the status to the trig pin
        } else {
           digitalWriteOnce(context, n, gTriggerDigitalOutChannel2, LOW);
           //analogWriteOnce(context, n / 2, gTriggerAnalogOutChannel, LOW); //write the status to the trig pin
        }
